//+------------------------------------------------------------------+
//|                                XAUUSD_Apex_Champion_v15.mq5     |
//|               Copyright 2026, Apex Institutional Trading             |
//|                 Master Champion Cent Edition (v15.0)              |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Apex Institutional Trading"
#property link      "https://www.mql5.com"
#property version   "15.00"
#property description "XAUUSD Apex Champion v15 (Tuned 2.0% Cent Risk + London/NY Session Filter)"
...
//--- Input Parameters ---
input group "=== Risk & Capital Sizing ==="
input double   InitialRiskPercent   = 2.0;       // Risk % per Trade (Tuned 2.0% for 650 USC)
input double   MaxAllowedPullback   = 15.0;      // Max Pullback % from High-Water Mark Peak
input double   MaxLotLimit          = 2.0;       // Safety Max Lot Limit
input ulong    MagicNumber          = 150001;    // Magic Number

input group "=== Daily Loss & Equity Protection ==="
input double   InpMaxDailyDrawdown  = 3.0;       // Max Daily Equity Drawdown Limit (%)
input int      InpMaxDailyLosses    = 2;         // Max Losses per Day -> Stop Day
...
input double   RSI_Buy_Max          = 72.0;      // Buy RSI Max
input double   RSI_Sell_Min         = 28.0;      // Sell RSI Min
...
input double   TP1_Close_Pct        = 35.0;      // TP1 Close % (35%)
input double   TP2_Close_Pct        = 35.0;      // TP2 Close % (35%)
input double   RR_Level_1           = 1.6;       // TP1 RR (1:1.6) -> Lock BE+
input double   RR_Level_2           = 3.2;       // TP2 RR (1:3.2) -> Lock 1.2 RR
input double   RR_Level_3           = 6.5;       // TP3 RR (1:6.5) -> Ultimate Runner

input group "=== Trading Sessions (GMT+7) ==="
input bool     UseTimeFilter        = true;      // Enable Session Window
input int      StartHour            = 11;        // Session Start Hour (London)
input int      EndHour              = 23;        // Session End Hour (NY Close)
...
int OnInit()
{
   ...
   {
      Print("Error creating indicator handles in v15");
      return INIT_FAILED;
   }
   ...
   Print("EA v15 Master Champion Initialized Successfully for ", _Symbol);
...
{
   for(int i = 0; i < 30; i++) {
      v15_states[i].ticket = 0;
      v15_states[i].tp1_done = false;
      v15_states[i].tp2_done = false;
      v15_states[i].initial_sl_dist = 0;
   }
}
...
void ExecuteOrder(int type, double atr_val)
{
   ...
   double tp_price = (type == 1) ? (entry + sl_dist_pts * RR_Level_3 * point) : (entry - sl_dist_pts * RR_Level_3 * point);
   ...
   // Pullback Protection Guard from All-Time High Peak
   ...
   {
      dynamic_risk = 1.0; // Halve risk to 1% to lock peak profits
   }
   ...
   if(type == 1)
   {
      if(m_trade.Buy(lot_size, _Symbol, entry, sl_price, tp_price, "Apex_v15_Buy"))
      {
   ...
   {
      if(m_trade.Sell(lot_size, _Symbol, entry, sl_price, tp_price, "Apex_v15_Sell"))
      {
         ulong ticket = m_trade.ResultOrder();
         RegisterState(ticket, sl_dist_pts * point);
      }
...
void RegisterState(ulong ticket, double sl_dist)
{
   for(int i = 0; i < 30; i++)
   {
      if(v15_states[i].ticket == 0)
      {
         v15_states[i].ticket = ticket;
         v15_states[i].tp1_done = false;
         v15_states[i].tp2_done = false;
         v15_states[i].initial_sl_dist = sl_dist;
         break;
      }
   }
}
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(m_posInfo.SelectByIndex(i))
      {
         if(m_posInfo.Symbol() == _Symbol && m_posInfo.Magic() == MagicNumber)
         {
            ...
            double profit = (m_posInfo.PositionType() == POSITION_TYPE_BUY) ? (current_price - entry) : (entry - current_price);
            if(profit <= 0) continue;
            
            double initial_risk = (idx >= 0 && v15_states[idx].initial_sl_dist > 0) ? v15_states[idx].initial_sl_dist : MathAbs(entry - current_sl);
            if(initial_risk <= 0) continue;
            
            double current_rr = profit / initial_risk;
            
            // TP1 Stage (Lock BE+ 0.3R)
            if(current_rr >= RR_Level_1 && (idx < 0 || !v15_states[idx].tp1_done))
            {
               double new_sl = (m_posInfo.PositionType() == POSITION_TYPE_BUY) ? NormalizeDouble(entry + (initial_risk * 0.3), _Digits) : NormalizeDouble(entry - (initial_risk * 0.3), _Digits);
               if(UsePartialClose) PartialClose(ticket, TP1_Close_Pct);
               m_trade.PositionModify(ticket, new_sl, current_tp);
               if(idx >= 0) v15_states[idx].tp1_done = true;
            }
            // TP2 Stage (Lock 1.2R)
            else if(current_rr >= RR_Level_2 && (idx < 0 || !v15_states[idx].tp2_done))
            {
               double new_sl = (m_posInfo.PositionType() == POSITION_TYPE_BUY) ? NormalizeDouble(entry + (initial_risk * 1.2), _Digits) : NormalizeDouble(entry - (initial_risk * 1.2), _Digits);
               if(UsePartialClose) PartialClose(ticket, TP2_Close_Pct);
               m_trade.PositionModify(ticket, new_sl, current_tp);
               if(idx >= 0) v15_states[idx].tp2_done = true;
            }
         }
      }
   }
...
int GetStateIndex(ulong ticket)
{
   for(int i = 0; i < 30; i++)
   {
      if(v15_states[i].ticket == ticket) return i;
   }
   return -1;
}
...
void PartialClose(ulong ticket, double pct)
{
   ...
   {
      double current_vol = m_posInfo.Volume();
      double min_lot = m_symInfo.LotsMin();
