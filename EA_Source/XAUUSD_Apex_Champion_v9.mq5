//+------------------------------------------------------------------+
//|    XAUUSD Apex Champion EA v9.0 (Dynamic Drawdown Guard)         |
//| Dual TF Trend (H4+H1) + Volatility Squeeze + Max Drawdown Guard |
//+------------------------------------------------------------------+
#property copyright "Antigravity EA Systems"
#property link      "https://www.mql5.com"
#property version   "9.00"
#property description "Ultra Smooth Equity Curve EA Built Natively for MT5 Strategy Tester"
#property strict
...
//--- Input Parameters
input group "=== Dynamic Capital Guard ==="
input double         BaseRiskPercent   = 2.0;         // Base Risk % per trade (2.0% Safe & Steady)
input double         MaxLotCap         = 10.0;        // Maximum Lot Cap per trade
input int            MinSL_Points      = 300;         // Minimum SL Points ($3.00)
input int            MaxSL_Points      = 500;         // Maximum SL Points ($5.00)
input double         ATR_SL_Multiplier = 1.6;         // Dynamic ATR SL Multiplier
input int            MagicNumber       = 900001;      // Magic Number

input group "=== Multi-Tier Profit Targets ==="
input bool           UsePartialClose   = true;        // Enable Partial Closure
input double         TP1_Close_Pct     = 40.0;        // TP1 Close % (40%)
input double         TP2_Close_Pct     = 40.0;        // TP2 Close % (40%)
input double         RR_Level_1        = 1.5;         // TP1 RR (1:1.5) -> Lock BE+
input double         RR_Level_2        = 3.0;         // TP2 RR (1:3.0) -> Lock 1.5 RR
input double         RR_Level_3        = 6.0;         // TP3 RR (1:6.0) -> Ultimate Runner

input group "=== Dual Timeframe Confluence ==="
input bool           UseDualTFFilter   = true;        // Enable H4 + H1 Dual Trend Filter
input int            H4_EMA_Fast       = 50;          // H4 Fast EMA
input int            H4_EMA_Slow       = 100;         // H4 Slow EMA
input int            H1_EMA_Fast       = 20;          // H1 Fast EMA
input int            H1_EMA_Slow       = 50;          // H1 Slow EMA
input int            M5_DonchianPeriod = 20;          // M5 Donchian Breakout Period
...
int handle_h4_fast, handle_h4_slow;
int handle_h1_fast, handle_h1_slow;
...
int OnInit()
{
    ...
    handle_h4_fast = iMA(_Symbol, PERIOD_H4, H4_EMA_Fast, 0, MODE_EMA, PRICE_CLOSE);
    handle_h4_slow = iMA(_Symbol, PERIOD_H4, H4_EMA_Slow, 0, MODE_EMA, PRICE_CLOSE);
    ...
    if(handle_h4_fast == INVALID_HANDLE || handle_h4_slow == INVALID_HANDLE ||
       handle_h1_fast == INVALID_HANDLE || handle_h1_slow == INVALID_HANDLE ||
    ...
    ResetStates();
    Print("EA v9.0 Apex Champion Initialized Successfully for ", _Symbol);
...
{
    IndicatorRelease(handle_h4_fast);
    IndicatorRelease(handle_h4_slow);
    ...
    Print("EA v9.0 Deinitialized. Reason: ", reason);
}

void ResetStates()
{
    for(int i = 0; i < 40; i++) {
        v9_states[i].ticket = 0;
        v9_states[i].tp1_done = false;
        v9_states[i].tp2_done = false;
        v9_states[i].initial_sl_dist = 0;
    }
}
...
void CheckSignal()
{
    ...
    int trend = GetDualTFTrend();
    if(UseDualTFFilter && trend == 0) return;
    ...
    double highest_h = GetHighestHigh(M5_DonchianPeriod, 2);
    double lowest_l  = GetLowestLow(M5_DonchianPeriod, 2);
...
int GetDualTFTrend()
{
    double h4_f[], h4_s[], h1_f[], h1_s[];
    if(CopyBuffer(handle_h4_fast, 0, 1, 1, h4_f) <= 0 ||
       CopyBuffer(handle_h4_slow, 0, 1, 1, h4_s) <= 0 ||
       CopyBuffer(handle_h1_fast, 0, 1, 1, h1_f) <= 0 ||
       CopyBuffer(handle_h1_slow, 0, 1, 1, h1_s) <= 0) return 0;
       
    double close_h4 = iClose(_Symbol, PERIOD_H4, 1);
    double close_h1 = iClose(_Symbol, PERIOD_H1, 1);
    
    bool h4_bull = (close_h4 > h4_s[0]) && (h4_f[0] > h4_s[0]);
    bool h1_bull = (close_h1 > h1_s[0]) && (h1_f[0] > h1_s[0]);
    
    bool h4_bear = (close_h4 < h4_s[0]) && (h4_f[0] < h4_s[0]);
    bool h1_bear = (close_h1 < h1_s[0]) && (h1_f[0] < h1_s[0]);
    
    if(h4_bull && h1_bull) return 1;    // Strong Dual TF Bullish
    if(h4_bear && h1_bear) return -1;   // Strong Dual TF Bearish
    return 0;
}
...
void ExecuteOrder(int type, double atr_val)
{
    ...
    double sl_dist_pts = (atr_val * ATR_SL_Multiplier) / symInfo.Point();
    ...
    double risk_amt = balance * (BaseRiskPercent / 100.0);
    double lot_size = CalculateLot(risk_amt, sl_dist_pts * symInfo.Point());
    
    if(type == 1) {
        if(trade.Buy(lot_size, _Symbol, entry, sl_price, tp_price, "XAUUSD v9 Apex Champion")) {
            ulong ticket = trade.ResultOrder();
            RegisterState(ticket, sl_dist_pts * symInfo.Point());
        }
    } else {
        if(trade.Sell(lot_size, _Symbol, entry, sl_price, tp_price, "XAUUSD v9 Apex Champion")) {
            ulong ticket = trade.ResultOrder();
            RegisterState(ticket, sl_dist_pts * symInfo.Point());
        }
    }
...
{
    for(int i = 0; i < 40; i++) {
        if(v9_states[i].ticket == 0) {
            v9_states[i].ticket = ticket;
            v9_states[i].tp1_done = false;
            v9_states[i].tp2_done = false;
            v9_states[i].initial_sl_dist = sl_dist;
            break;
        }
    }
}
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        if(posInfo.SelectByIndex(i)) {
            if(posInfo.Symbol() == _Symbol && posInfo.Magic() == MagicNumber) {
                ...
                double initial_risk = (idx >= 0 && v9_states[idx].initial_sl_dist > 0) ? v9_states[idx].initial_sl_dist : MathAbs(entry - current_sl);
                if(initial_risk <= 0) continue;
                
                double current_rr = profit / initial_risk;
                
                // TP1 Stage (Lock BE+)
                if(current_rr >= RR_Level_1 && (idx < 0 || !v9_states[idx].tp1_done)) {
                    ...
                    if(idx >= 0) v9_states[idx].tp1_done = true;
                }
                // TP2 Stage (Lock 1.5 RR)
                else if(current_rr >= RR_Level_2 && (idx < 0 || !v9_states[idx].tp2_done)) {
                    ...
                    trade.PositionModify(ticket, new_sl, current_tp);
                    if(idx >= 0) v9_states[idx].tp2_done = true;
                }
            }
        }
    }
...
{
    for(int i = 0; i < 40; i++) {
        if(v9_states[i].ticket == ticket) return i;
    }
    return -1;
}
