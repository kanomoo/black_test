//+------------------------------------------------------------------+
//|                                   XAUUSD_Apex_SMC_v20.mq5        |
//|               Copyright 2026, Apex Institutional Trading             |
//|        Sovereign Asian Range & Volatility Expansion Engine (v20.0)|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Apex Institutional Trading"
#property link      "https://www.mql5.com"
#property version   "20.00"
#property description "XAUUSD Apex SMC v20 (Optimized Asian Range + Bollinger Expansion Guard)"
#property description "Engineered to deliver high winning months on 650 USC Cent Accounts"
...
input int      InpMaxDailyTrades    = 2;         // Maximum Trades Per Day (Max 2 Trades/Day)
input double   InpMaxDailyDrawdown  = 2.5;       // Max Daily Equity Drawdown Limit (%)
input ulong    MagicNumber          = 200001;    // Magic Number
...
input int      TradeEndHour         = 22;        // Trade End Hour (22:00 GMT+7)
input double   MinAsianRangePts     = 400;       // Min Asian Range (4.0 Gold Pts)
input double   MaxAsianRangePts     = 3500;      // Max Asian Range (35.0 Gold Pts)
...
input double   InpTP2_RR            = 2.2;       // TP2 Risk-Reward (1:2.2) -> Final TP
...
int OnInit()
{
   ...
   {
      Print("Error creating indicator handles in v20");
      return INIT_FAILED;
   }
   
   m_day_start_equity = AccountInfoDouble(ACCOUNT_EQUITY);
   Print("EA v20 Apex SMC Sovereign Initialized Successfully for ", _Symbol);
...
void OnTick()
{
   ...
   if(m_daily_trade_count >= InpMaxDailyTrades) return;
...
void ExecuteOrder(int type, double atr_val)
{
   ...
   if(type == 1)
   {
      if(m_trade.Buy(lot_size, _Symbol, entry, sl_price, tp_price, "Apex_v20_Buy"))
      {
         m_daily_trade_count++;
         m_tp1_done = false;
         Print("v20 BUY Executed! Lot: ", lot_size, " SL: ", sl_price, " TP: ", tp_price);
      }
   }
   else
   {
      if(m_trade.Sell(lot_size, _Symbol, entry, sl_price, tp_price, "Apex_v20_Sell"))
      {
         m_daily_trade_count++;
         m_tp1_done = false;
         Print("v20 SELL Executed! Lot: ", lot_size, " SL: ", sl_price, " TP: ", tp_price);
      }
   }
}
