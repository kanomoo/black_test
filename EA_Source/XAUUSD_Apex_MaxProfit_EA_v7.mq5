//+------------------------------------------------------------------+
//|       XAUUSD Apex MaxProfit EA v7.0 (Ultimate Max Profit)        |
//|    Triple-Confluence (H1/M15/M5) + Multi-Stage Compound Profit   |
//+------------------------------------------------------------------+
#property copyright "Antigravity EA Systems"
#property link      "https://www.mql5.com"
#property version   "7.00"
#property description "Ultimate Max Profit EA Built Natively for MT5 Strategy Tester"
#property strict
...
//--- Input Parameters
input group "=== MaxProfit Risk & Sizing ==="
...
input int            MagicNumber       = 700001;      // Magic Number
...
input double         RR_Level_3        = 7.0;         // TP3 RR (1:7.0) -> Ultimate Runner
...
int OnInit()
{
    ...
    ResetStates();
    Print("EA v7.0 Apex MaxProfit Initialized Successfully for ", _Symbol);
...
void OnDeinit(const int reason)
{
    ...
    Print("EA v7.0 Deinitialized. Reason: ", reason);
}

void ResetStates()
{
    for(int i = 0; i < 40; i++) {
        v7_states[i].ticket = 0;
        v7_states[i].tp1_done = false;
        v7_states[i].tp2_done = false;
        v7_states[i].initial_sl_dist = 0;
    }
}
...
void CheckSignal()
{
    ...
    bool buy_signal  = (h1_trend >= 0) && (close_1 > highest_h) && (close_1 > open_1) && (rsi[0] > 54.0 && rsi[0] < 75.0);
    bool sell_signal = (h1_trend <= 0) && (close_1 < lowest_l)  && (close_1 < open_1) && (rsi[0] < 46.0 && rsi[0] > 25.0);
...
int GetH1Trend()
{
    ...
    if(close_h1 > s[0] && f[0] > m[0] && m[0] > s[0]) return 1;    // Strong Bullish Trend
    if(close_h1 < s[0] && f[0] < m[0] && m[0] < s[0]) return -1;   // Strong Bearish Trend
    return 0;
}
...
void ExecuteOrder(int type, double atr_val)
{
    ...
    if(type == 1) {
        if(trade.Buy(lot_size, _Symbol, entry, sl_price, tp_price, "XAUUSD v7 Apex MaxProfit")) {
            ulong ticket = trade.ResultOrder();
            RegisterState(ticket, sl_dist_pts * symInfo.Point());
        }
    } else {
        if(trade.Sell(lot_size, _Symbol, entry, sl_price, tp_price, "XAUUSD v7 Apex MaxProfit")) {
            ulong ticket = trade.ResultOrder();
            RegisterState(ticket, sl_dist_pts * symInfo.Point());
        }
    }
...
{
    for(int i = 0; i < 40; i++) {
        if(v7_states[i].ticket == 0) {
            v7_states[i].ticket = ticket;
            v7_states[i].tp1_done = false;
            v7_states[i].tp2_done = false;
            v7_states[i].initial_sl_dist = sl_dist;
            break;
        }
    }
}
    ...
    for(int i = 0; i < PositionsTotal(); i++) {
        if(posInfo.SelectByIndex(i)) {
            if(posInfo.Symbol() == _Symbol && posInfo.Magic() == MagicNumber) {
                ...
                double initial_risk = (idx >= 0 && v7_states[idx].initial_sl_dist > 0) ? v7_states[idx].initial_sl_dist : MathAbs(entry - current_sl);
                if(initial_risk <= 0) continue;
                
                double current_rr = profit / initial_risk;
                
                // TP1 Stage (Lock BE+)
                if(current_rr >= RR_Level_1 && (idx < 0 || !v7_states[idx].tp1_done)) {
                    ...
                    if(idx >= 0) v7_states[idx].tp1_done = true;
                }
                // TP2 Stage (Lock 1.5 RR)
                else if(current_rr >= RR_Level_2 && (idx < 0 || !v7_states[idx].tp2_done)) {
                    ...
                    trade.PositionModify(ticket, new_sl, current_tp);
                    if(idx >= 0) v7_states[idx].tp2_done = true;
                }
            }
        }
    }
...
{
    for(int i = 0; i < 40; i++) {
        if(v7_states[i].ticket == ticket) return i;
    }
    return -1;
}
