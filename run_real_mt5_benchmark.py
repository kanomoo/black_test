...
# Copy all EAs to MT5 Experts folder
for ea in ea_files:
...
for idx, ea in enumerate(ea_files, 1):
    print(f"\n[{idx}/{len(ea_files)}] Executing MT5 Native Strategy Tester for {ea}...")
    
    subprocess.run("taskkill /F /IM terminal64.exe /T 2>nul", shell=True)
    time.sleep(1.0)
    
    report_filename = f"Report_REAL_MT5_{ea.replace('.ex5', '')}.html"
    report_full_path = os.path.join(report_dir, report_filename)
    if os.path.exists(report_full_path):
        try: os.remove(report_full_path)
        except: pass
        
    ini_path = os.path.join(mt5_dir, f"test_real_{ea.replace('.ex5', '')}.ini")
    
    # Visual=0 for fast native MT5 execution, Deposit=750 (750 USC = $7.50 USD), Currency=USD
...
"""
    ...
    # Wait up to 90s for terminal to complete test and exit cleanly
    start_t = time.time()
    ...
    parsed = parse_mt5_html_report(report_full_path)
    ...
    if parsed:
        ...
        print(f"   --> SUCCESS: Net Profit: {net_profit_val:+.2f} (USC) | Trades: {trades} | PF: {pf} | Max DD: {max_dd}")
...
out_json = r"D:\Trade_Gus\Results_Data\mt5_official_real_benchmark_750usc.json"
os.makedirs(os.path.dirname(out_json), exist_ok=True)
with open(out_json, "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2)

print("\n" + "="*115)
print("  OFFICIAL MT5 STRATEGY TESTER BENCHMARK SUMMARY (750 USC = $7.50 USD / 1 WEEK)  ")
print("="*115)
