...
ea_source = r"D:\Trade_Gus\EA_Source\XAUUSD_Apex_Champion_v15.ex5"
ea_name = "XAUUSD_Apex_Champion_v15.ex5"
...
print("==========================================================================")
print("  RUNNING MT5 MONTHLY BACKTEST FOR EA V15 (DEPOSIT: 650 USC | 2024-2026) ")
print("==========================================================================")
...
"""
    ...
    if parsed:
        ...
        print(f"   [{label}] Net Profit: ${net_profit_val:+.2f} USC | Trades: {trades} | PF: {pf} | Max DD: {max_dd} | Win Rate: {win_rate_str}")
...
out_json = r"D:\Trade_Gus\Results_Data\v15_650usc_monthly_backtest_results.json"
os.makedirs(os.path.dirname(out_json), exist_ok=True)
with open(out_json, "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2)

total_net_profit = sum(r["NetProfit"] for r in results)
winning_months = sum(1 for r in results if r["NetProfit"] > 0)
losing_months = sum(1 for r in results if r["NetProfit"] < 0)

print("\n" + "="*100)
print(f"  SUMMARY OF V15 MONTHLY BACKTEST RESULTS (DEPOSIT: 650 USC | 2024-08 TO 2026-08)  ")
print("="*100)
for r in results:
    print(f"Month: {r['Month']:<10} | NetProfit: ${r['NetProfit']:<+10.2f} USC | Trades: {r['Trades']:<5} | PF: {r['ProfitFactor']:<6} | MaxDD: {r['MaxDrawdown']:<10} | WinRate: {r['WinRate']}")
print("="*100)
print(f"TOTAL NET PROFIT: ${total_net_profit:+.2f} USC ({(total_net_profit/650.0)*100:+.2f}%)")
print(f"WINNING MONTHS: {winning_months}/{len(results)} ({winning_months/len(results)*100:.1f}%) | LOSING MONTHS: {losing_months}/{len(results)}")
print("="*100)
